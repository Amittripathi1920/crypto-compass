...
import { organizationInNeonAuth, invitationInNeonAuth, userInNeonAuth, sessionInNeonAuth, accountInNeonAuth, memberInNeonAuth, trackedTrades } from "./schema";
...
export const userInNeonAuthRelations = relations(userInNeonAuth, ({many}) => ({
	...
	memberInNeonAuths: many(memberInNeonAuth),
	trackedTrades: many(trackedTrades),
}));
...
export const trackedTradesRelations = relations(trackedTrades, ({one}) => ({
	userInNeonAuth: one(userInNeonAuth, {
		fields: [trackedTrades.userId],
		references: [userInNeonAuth.id]
	}),
}));