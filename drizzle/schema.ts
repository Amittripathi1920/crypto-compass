import { pgTable, pgSchema, index, foreignKey, uuid, text, timestamp, unique, boolean, uniqueIndex, jsonb, doublePrecision, bigint } from "drizzle-orm/pg-core"
...
export const trackedTrades = pgTable("tracked_trades", {
	...
	leverage: doublePrecision().notNull(),
	...
	foreignKey({
			columns: [table.userId],
			foreignColumns: [userInNeonAuth.id],
			name: "tracked_trades_user_id_user_id_fk"
		}).onDelete("cascade"),
]);
